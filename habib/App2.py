...
class VideoPlayer(QWidget):
    def __init__(self):
        ...
        self.lbl_info_header.setStyleSheet("font-weight: bold; color: #ddd; margin-bottom: 5px;")
        self.lbl_info_header.setFixedHeight(20)

        self.info_label = QLabel("Status: Menunggu video...")
        self.info_label.setStyleSheet("font-size: 13px; color: white; background-color: #333; padding: 10px; border-radius: 4px;")
    ...
    def toggle_log_view(self):
        if not self.is_showing_log:
            ...
            self.lbl_info_header.setText("LOG DETIL KENDARAAN")
    ...
    def run_detection(self):
        ...
        if not coords_list:
            self.info_label.setText("Gambar garis deteksi terlebih dahulu!")
            return
        ...
        cap.release()
        ww = max(1, self.video_widget.width())
        wh = max(1, self.video_widget.height())
        sx, sy = vw/ww, vh/wh
        
        coords_scaled = []
        for item in coords_list:
            x1, y1, x2, y2 = item["coords"]
            coords_scaled.append((int(x1*sx), int(y1*sy), int(x2*sx), int(y2*sy)))

        self.worker = DetectionWorker(self.current_video_path, "model/best.pt", "hasil.mp4", coords_scaled)
        self.worker.progress_changed.connect(lambda v: self.info_progress.setValue(v))
